...
class MultiSourceFusionModel:
    ...
    def train_prophet_model(self, df):
        ...
        # Prepare data for Prophet - ensure timezone is removed
    ...
    def train_ensemble(self, X, y, features, test_size=0.2):
        ...
        # Prepare Prophet data - ensure we have proper dates
        prophet_data = pd.DataFrame({
            'Close': y_train_final.values,
            'Date': y_train_final.index
        }).set_index('Date')
        
        # Ensure Prophet gets enough data
        if len(prophet_data) > 30:
            prophet_model = self.train_prophet_model(prophet_data)
            self.models['prophet'] = prophet_model
        else:
            print("Insufficient data for Prophet model. Using XGBoost and GRU only.")
    ...
    def evaluate_models(self, X_test, y_test, features):
        ...
        # 3. Prophet predictions (if available)
        if self.models['prophet'] is not None:
            # Prepare future dates without timezone
            ...
            prophet_pred = prophet_forecast.set_index('ds')['yhat'].reindex(
                X_test.index.tz_localize(None) if X_test.index.tz is not None else X_test.index
            )
            
            # Handle any missing values
            prophet_pred = prophet_pred.fillna(prophet_pred.mean() if not prophet_pred.empty else 0)
            ...
            # If Prophet model is not available, use neutral predictions
            prophet_pred = pd.Series(0, index=y_test.index)
            metrics['prophet'] = {
                'mae': np.mean(np.abs(y_test)),
        ...
        maes = [xgb_mae, gru_mae, metrics['prophet']['mae']]
        ...
        if self.models['prophet'] is not None:
            print(f"Prophet - MAE: {metrics['prophet']['mae']:.6f}, RMSE: {metrics['prophet']['rmse']:.6f}, R²: {metrics['prophet']['r2']:.4f}")
        ...
        with st.spinner("Running comprehensive research analysis..."):
            ...
            **Feature Engineering Complete:**
            ...
            for i in range(forecast_days):
                ...
                # Prophet prediction (if available)
                if models['prophet'] is not None:
