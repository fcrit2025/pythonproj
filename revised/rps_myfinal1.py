...
from tensorflow.keras.layers import GRU, Dense, Dropout, LSTM, Bidirectional, Input, Concatenate
...
# Enhanced Indian stock symbols with sector information
@st.cache_data
...
def get_fii_dii_data(days=30):
    """Fetch FII/DII data - Simplified implementation"""
    try:
        # This is a placeholder - you would need to implement actual API calls
        # or web scraping for Groww FII/DII data
...
# Enhanced stock data fetching with market indicators
def get_stock_data(ticker, start, end):
    ...
    try:
        ...
        # Add FII/DII data
        fii_dii_data = get_fii_dii_data(len(data))
        if not fii_dii_data.empty:
            data = data.join(fii_dii_data, how='left')
...
def create_advanced_features(df):
    ...
    df['MA_Trend'] = (df['5D_MA'] > df['20D_MA']).astype(int)
    
    # Volatility features
    df['5D_Volatility'] = df['Returns'].rolling(5).std()
    df['20D_Volatility'] = df['Returns'].rolling(20).std()
    ...
    if 'India_VIX' in df.columns:
        ...
        df['VIX_Level'] = pd.cut(df['India_VIX'], 
                               bins=[0, 15, 25, 100], 
...
def create_enhanced_hybrid_model(df_stock, sentiment_features):
    ...
    gru_model = Sequential([
        GRU(64, input_shape=(1, X_train_3d.shape[2]), return_sequences=True),
        Dropout(0.3),
    ...
    gru_model.fit(X_train_3d, y_train, epochs=100, batch_size=32, 
    ...
    lstm_model.fit(X_train_3d, y_train, epochs=100, batch_size=32, 
                  validation_data=(X_test_3d, y_test), callbacks=callbacks, verbose=0)
    lstm_pred = lstm_model.predict(X_test_3d).flatten()
    models['lstm'] = lstm_model
    predictions['lstm'] = lstm_pred
    
    # 4. BiLSTM Model
    bilstm_model = Sequential([
        Bidirectional(LSTM(32, return_sequences=True), input_shape=(1, X_train_3d.shape[2])),
        Dropout(0.3),
    ...
    bilstm_model.fit(X_train_3d, y_train, epochs=100, batch_size=32, 
                    validation_data=(X_test_3d, y_test), callbacks=callbacks, verbose=0)
    bilstm_pred = bilstm_model.predict(X_test_3d).flatten()
...
# Enhanced forecasting with market indicators
def enhanced_hybrid_predict_prices(models, scaler, last_known_data, features, days=10):
    """Generate enhanced predictions with market indicators"""
    try:
        ...
        for i, date in enumerate(future_dates):
            # Prepare input data
            current_features = current_data[features].iloc[-1:].copy()
            ...
            # Calculate weighted ensemble (simple average for demo)
            weights = {'xgb': 0.25, 'gru': 0.2, 'lstm': 0.2, 'bilstm': 0.2, 'prophet': 0.15}
            final_pred = 0
            total_weight = 0
            ...
            future_prices.loc[date, 'Confidence'] = 75  # Base confidence
...
# Enhanced Streamlit UI with comprehensive analysis
def main():
    ...
    **Academic Research-Grade Analysis** featuring:
    ...
    - **Market Indicators**: India VIX, FII/DII Data, News Sentiment
    ...
    analysis_type = st.sidebar.radio("Analysis Type", 
                                   ["Comprehensive Analysis", "Technical Only", "With Market Data"])
    
    if st.sidebar.button("🚀 Run Advanced Analysis", type="primary"):
        with st.spinner("Running comprehensive AI analysis..."):
            analyze_stock(selected_stock, start_date, end_date, analysis_type)

def analyze_stock(selected_stock, start_date, end_date, analysis_type):
    ...
    display_stock_info(ticker, df_stock)
    
    # Display charts
    st.header("📈 Price Analysis")
    display_price_charts(df_stock, selected_stock)
    
    # Market data analysis
    if analysis_type != "Technical Only":
    ...
    with st.spinner("Training 5 AI models... This may take a few minutes."):
        df_enhanced, models, scaler, features, metrics = create_enhanced_hybrid_model(
            df_stock, {})  # Empty sentiment for now
    ...
    if not future_prices.empty:
        display_forecast_results(future_prices, df_stock, selected_stock, metrics)
...
def display_model_comparison(metrics):
    ...
    for model, name in zip(models_list, model_names):
        if model in metrics:
            ...
            metrics_data.append({
                ...
                'Weight': f"{model_metrics.get('weights', {}).get(model, 0)*100:.1f}%" if model != 'ensemble' else 'N/A'
            })
    ...
    **🎯 Performance Summary:**
    ...
    - **Ensemble Advantage**: Combines strengths of all models
    - **R² Score**: {metrics.get('ensemble', {}).get('r2', 0):.3f} (closer to 1 is better)
    """)
...
def display_forecast_results(future_prices, df_stock, selected_stock, metrics):
    ...
    st.dataframe(forecast_display.style.applymap(style_forecast, subset=['Daily Change (%)']))
...
def display_investment_recommendation(future_prices, df_stock, metrics):
    ...
    # Risk factors
    st.subheader("⚠️ Risk Factors Considered")
    
    risk_factors = [
        f"Market Volatility (VIX): {'High' if 'India_VIX' in df_stock.columns and df_stock['India_VIX'].iloc[-1] > 25 else 'Medium' if df_stock['India_VIX'].iloc[-1] > 15 else 'Low'}",
        f"Model Consensus: {len([m for m in metrics.keys() if m != 'ensemble' and metrics[m].get('accuracy', 0) > 65])}/4 models confident",
        f"Price Trend: {'Bullish' if price_change_pct > 0 else 'Bearish'}",
        f"Uncertainty Range: ±{100 - ensemble_accuracy:.1f}%"
    ]
